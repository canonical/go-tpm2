...
func (s *authSuite) testPolicySignedAuthorization(c *C, data *testPolicySignedAuthorizationData) {
	...
	auth := NewPolicySignedAuthorization(nonceTPM, data.cpHashA, data.expiration)
	c.Check(auth.NonceTPM, DeepEquals, nonceTPM)
	c.Check(auth.CpHash, DeepEquals, data.cpHashA)
	...
	_, _, err = s.TPM.PolicySigned(key, session, data.includeNonceTPM, data.cpHashA, data.policyRef, data.expiration, auth.Authorization.Signature)
	c.Check(err, IsNil)
}
...
func (s *authSuite) TestPolicySignedAuthorizationWithCpHash(c *C) {
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	c.Assert(err, IsNil)
	...
	cpHashA, err := ComputeCpHash(tpm2.HashAlgorithmSHA256, tpm2.CommandUnseal, []Named{objectutil.NewSealedObjectTemplate()})
	c.Assert(err, IsNil)

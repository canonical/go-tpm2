	...
	for _, policy := range policies {
		...
		candidatePolicies = append(candidatePolicies, &authorizedPolicy{
			policyBranch: policyBranch{
				...
				PolicyDigests: tpm2.TaggedHashList{tpm2.MakeTaggedHash(runner.session().HashAlg(), digest)},
			},
...
func (d policyElementDetails) RawOR() *policyRawORElement {
	return union.ContentsPtr[policyRawORElement](d.contents)
}
...
func (d policyElementDetails) SelectMarshal(selector any) any {
	...
	case commandRawPolicyOR:
		return union.ContentsMarshal[policyRawORElement](d.contents)
	default:
		return nil
	}
}
...
func (d *policyElementDetails) SelectUnmarshal(selector any) any {
	...
	case commandRawPolicyOR:
		return union.ContentsUnmarshal[policyRawORElement](&d.contents)
	default:
		return nil
	}
}
...
func (r *policyComputeRunner) runBranch(branches policyBranches) (selected int, err error) {
	...
	for i, branch := range branches {
		...
		for j, digest := range branch.PolicyDigests {
			...
			branch.PolicyDigests[j] = tpm2.MakeTaggedHash(r.session().HashAlg(), computedDigest)
			added = true
			break
		}
		if !added {
			branch.PolicyDigests = append(branch.PolicyDigests, tpm2.MakeTaggedHash(r.session().HashAlg(), computedDigest))
		}
	}
...
func (p *Policy) AddDigest(alg tpm2.HashAlgorithmId) (tpm2.Digest, error) {
	...
	for i, d := range policy.PolicyDigests {
		if d.HashAlg == alg {
			policy.PolicyDigests[i] = tpm2.MakeTaggedHash(alg, computedDigest)
			addedDigest = true
			break
		}
	}
	if !addedDigest {
		policy.PolicyDigests = append(policy.PolicyDigests, tpm2.MakeTaggedHash(alg, computedDigest))
	}

...
//
// The auth parameter should be a HandleContext corresponding to either HandlePlatform or HandleOwner. This command requires
// authorization of auth with the user role, provided via authAuth.
//
...
func (t *TPMContext) SetCommandCodeAuditStatus(auth HandleContext, auditAlg HashAlgorithmId, setList, clearList CommandCodeList, authAuth interface{}, sessions ...*Session) error {
	return t.RunCommand(CommandSetCommandCodeAuditStatus, sessions,
		HandleContextWithAuth{Context: auth, Auth: authAuth}, Separator,

...
func TestNVUndefineSpaceSpecial(t *testing.T) {
	tpm, _ := testutil.NewTPMContextT(t, testutil.TPMFeaturePlatformPersist|testutil.TPMFeatureChangePlatformAuth)
	defer closeTPM(t, tpm)
...
func TestNVReadLock(t *testing.T) {
	tpm, _ := testutil.NewTPMContextT(t, testutil.TPMFeatureOwnerPersist|testutil.TPMFeatureStClearChange)
	defer closeTPM(t, tpm)

...
func (s *ppiSuite) TestPPIStateTransitionActionShutdown(c *C) {
	pp := New("1.3", &mockPPIBackend{sta: ppi.StateTransitionShutdownRequired})
	action, err := pp.StateTransitionAction()
	c.Check(err, IsNil)
...
func (s *ppiSuite) TestPPIOperationStatusDisableTPM(c *C) {
	pp := New("1.3", &mockPPIBackend{ops: map[ppi.OperationId]ppi.OperationStatus{ppi.OperationDisableTPM: ppi.OperationFirmwareOnly}})
...
func (s *ppiSuite) TestPPIOperationStatusClearTPM(c *C) {
	pp := New("1.3", &mockPPIBackend{ops: map[ppi.OperationId]ppi.OperationStatus{ppi.OperationClearTPM: ppi.OperationPPRequired}})

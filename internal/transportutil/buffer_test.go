...
func (s *bufferSuite) TestBufferCommands(c *C) {
	...
	_, err := mu.MarshalToWriter(BufferCommands(w, 4096), hdr, mu.MakeRaw(internal_testutil.DecodeHexString(c, "0000")))
	c.Check(err, IsNil)
...
func (s *bufferSuite) TestBufferCommandsShortWrite(c *C) {
	...
	w := &countingWriter{buf: new(bytes.Buffer)}
	_, err := mu.MarshalToWriter(BufferCommands(w, 4096), hdr, mu.MakeRaw(internal_testutil.DecodeHexString(c, "00000000")))

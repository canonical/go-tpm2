...
// Certify executes the TPM2_Certify command, which is used to prove that
// an object with a specific name is loaded in to the TPM. By producing an
// attestation, the TPM certifies that the object with a given name is
// loaded in to the TPM and consistent with a valid sensitive area.
//
// The objectContext parameter corresponds to the object for which to
// produce an attestation. The command requires authorization with the
// admin role for objectContext, with session based authorization provided
// via objectContextAuthSession.
//
// If signContext is not nil, the returned attestation will be signed by
// the key associated with it. This command requires authorization with
// the user auth role for signContext, with session based authorization
// provided via signContextAuthSession.
//
// If signContext is not nil and the object associated with signContext is
// not a signing key, a *[TPMHandleError] error with an error code of
// [ErrorKey] will be returned for handle index 2.
//
// If signContext is not nil and if the scheme of the key associated with
// signContext is [AsymSchemeNull], then inScheme must be provided to
// specify a valid signing scheme for the key. If it isn't, a
// *[TPMParameterError] error with an error code of [ErrorScheme] will be
...
// else a *[TPMParameterError] error with an error code of [ErrorScheme]
// will be returned for parameter index 2.
//
// If successful, it returns an attestation structure detailing the name
// of the object associated with objectContext. If signContext is not nil,
// the attestation structure will be signed by the associated key and
// returned too.
...
// CertifyCreation executes the TPM2_CertifyCreation command, which is
// used to prove the association between the object represented by
// objectContext and its creation data represented by creationHash. It
// does this by computing a ticket from creationHash and the name of
// the object represented by objectContext and then verifying that it
// matches the provided creationTicket, which was provided by the TPM at
...
// provided via signContextAuthSession.
//
// If signContext is not nil and the object associated with signContext
// is not a signing key, a *[TPMHandleError] error with an error code of
// [ErrorKey] will be returned for handle index 1.
//
// If signContext is not nil and if the scheme of the key associated with
// signContext is [AsymSchemeNull], then inScheme must be provided to
// specify a valid signing scheme for the key. If it isn't, a
...
//
// If creationTicket corresponds to an invalid ticket, a
// *[TPMParameterError] error with an error code of [ErrorTicket] will be
// returned for parameter index 4.
//
// If the digest generated for signing is greater than or has a larger
// size than the modulus of the key associated with signContext, a
// *[TPMError] with an error code of [ErrorValue] will be returned.
//
// If successful, it returns an attestation structure. If signContext is
// not nil, the attestation structure will be signed by the associated
// key and returned too.
...
// Quote executes the TPM2_Quote command in order to quote a set of PCR
// values. The TPM will hash the set of PCRs specified by the pcrs
// parameter.
//
// If signContext is not nil, the returned attestation will be signed by
// the key associated with it. This command requires authorization with
...
// will be returned for parameter index 2.
//
// This function will call [TPMContext.InitProperties] if it hasn't
// already been called.
//
// On success, it returns an attestation structure containing the hash of
// the PCRs selected by the pcrs parameter. If signContext is not nil, the
// attestation structure will be signed by the associated key and returned
// too.
...
// GetSessionAuditDigest executes the TPM2_GetSessionAuditDigest to obtain
...
// will be returned for parameter index 2.
//
// On success, it returns an attestation structure detailing the current
// audit digest for sessionContext. If signContext is not nil, the
// attestation structure will be signed by the associated key and returned
// too.
...
// GetCommandAuditDigest executes the TPM2_GetCommandAuditDigest command
...
// the key associated with it. This command requires authorization with
// the user auth role for signContext, with session based authorization
// provided via provided via signContextAuthSession.
//
// If signContext is not nil and the object associated with signContext is
// not a signing key, a *[TPMHandleError] error with an error code of
// [ErrorKey] will be returned for handle index 2.
...
//
// On success, it returns an attestation structure detailing the current
// command audit digest, digest algorithm and a digest of the list of
// commands being audited. If signContext is not nil, the attestation
...
// signContext is [AsymSchemeNull], then inScheme must be provided to
// specify a valid signing scheme for the key. If it isn't, a
// *[TPMParameterError] error with an error code of [ErrorScheme] will
// be returned for parameter index 2.
//
// If signContext is not nil and the scheme of the key associated with
// signContext is not [AsymSchemeNull], then inScheme may be nil. If it
// is provided, then the specified scheme must match that of the signing
// key, else a *[TPMParameterError] error with an error code of
// [ErrorScheme] will be returned for parameter index 2.
//
// On success, it returns an attestation structure detailing the current
// values of time and clock. If signContext is not nil, the attestation
// structure will be signed by the associated key and returned too.

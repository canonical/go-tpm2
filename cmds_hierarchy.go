...
// primary object in the hierarchy corresponding to primaryObject.
//
// The primaryObject parameter should correspond to a hierarchy. The command
// requires authorization with the user auth role for primaryObject, with
// session based authorization provided via primaryObjectAuthSession.
//
...
// defines the digest algorithm for computing the name of the object. The Attrs
// field defines the attributes of the object. The AuthPolicy field allows an
// authorization policy to be defined for the new object.
//
// Data that will form part of the sensitive area of the object can be provided
// via inSensitive, which is optional.
//
...
// then the length of the digest must match the name algorithm selected via
// the NameAlg field, else a *TPMParameterError error with an error code of
// ErrorSize is returned for parameter index 2.
//
// If the scheme in the Params field of inPublic is inappropriate for the
// usage, a *TPMParameterError errow with an error code of ErrorScheme will
// be returned for parameter index 2.
//
// If the digest algorithm specified by the scheme in the Params field of
// inPublic is inappropriate for the usage, a *TPMParameterError error with
...
//
// If the Type field of inPublic is ObjectTypeSymCipher and the key size is
// an unsupported value, a *TPMError with an error code of ErrorKeySize will be
...
// been called.
//
// On success, a ResourceContext instance will be returned that corresponds to
// the newly created object on the TPM. It will not be necessary to call
// ResourceContext.SetAuthValue on it - this function sets the correct
// authorization value so that it can be used in subsequent commands that
// require knowledge of the authorization value. If the Type field of inPublic
// is ObjectTypeKeyedHash or ObjectTypeSymCipher, then the returned *Public
// object will have a Unique field that is the digest of the sensitive data and
// the value of the object's seed in the sensitive area, computed using the
// object's name algorithm. If the Type field of inPublic is ObjectTypeECC or
// ObjectTypeRSA, then the returned *Public object will have a Unique field
// containing details about the public part of the key, computed from the
// private part of the key.
//
// The returned *CreationData will contain a digest computed from the values
// of PCRs selected by the creationPCR parameter at creation time in the
// PCRDigest field. It will also contain the provided outsideInfo in the
// OutsideInfo field. The returned *TkCreation ticket can be used to prove
// the association between the created object and the returned *CreationData
// via the TPMContext.CertifyCreation method.

...
// digest, using the key associated with keyContext. On success, a valid TkVerified structure will be returned. If the signature is
...
// scheme for the key. If the scheme of the key associated with keyContext is not AlgorithmNull, then inScheme may be nil. If it is
...
// then validation may be nil. If it is provided, it must be a valid ticket.
